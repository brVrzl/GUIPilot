    ...
    for annotation in annotation_list:
        ...
        if image_name not in image_dict:
            ...
            coco_format["images"].append(
                {"id": image_id, "width": width, "height": height, "file_name": image_name}
            )
            image_id += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./yolo/dataset.yaml", help="dataset configuration"
    )
    parser.add_argument(
        "--json_train",
        type=str,
        default="./datasets/80app_train_groundtruth.json",
        help="save the training data as json",
    )
    parser.add_argument(
        "--json_test",
        type=str,
        default="./datasets/80app_test_groundtruth.json",
        help="save the test data as json",
    )
    parser.add_argument(
        "--model_name",
        type=str,
    ...
    # Load the YAML file
    with open(args.config) as file:  # Replace with the actual file path
                ...
                if file.endswith("json"):
                    ...
                    if ct < num_train:  # training
                        Widget.write_yolo_labels(
                            os.path.join(
                                train_label_dir, app_dir + "_" + file.replace("json", "txt")
                            ),
                            label2id,
                            ws,
                            image_width,
                        ...
                        for shape in data["shapes"]:
                            new_annot = {}
                            new_annot["image_name"] = os.path.join(
                                ground_truth_dir, app_dir, image_path
                            )
                        ...
                        shutil.copy(
                            os.path.join(ground_truth_dir, app_dir, image_path),
                            os.path.join(train_img_dir, app_dir + "_" + image_path),
                        )
                    else:
                        Widget.write_yolo_labels(
                            os.path.join(
                                val_label_dir, app_dir + "_" + file.replace("json", "txt")
                            ),
                            label2id,
                            ws,
                            image_width,
                    ...
                    if image_name in image_dict:
                        ...
                        for it in range(len(scores)):
                            ...
                            prediction = {
                                ...
                                "score": float(
                                    scores[it]
                                ),  # Ensure score is provided in annotation_list
                            }
